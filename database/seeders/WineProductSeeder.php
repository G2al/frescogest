...
class WineProductSeeder extends Seeder
{
    public function run(): void
    {
        ...
        foreach ($this->catalog() as $group) {
            ...
            if ($group['producer'] !== null) {
                $producer = Producer::query()->firstOrCreate(
                    ['product_category_id' => $category->id, 'name' => $group['producer']],
                    ['sort_order' => ++$producerSortOrder, 'active' => true],
                );
            }

            foreach ($group['wines'] as $wine) {
                $code = 'IPF-WINE-'.strtoupper(substr(
                    sha1(($index + 1).'|'.($group['producer'] ?? '').'|'.$wine['name']),
                    0,
                    8,
                ));
    ...
    /**
     * Catalogo reale trascritto dal PDF fornito dal cliente, raggruppato per casa
     * produttrice così come compare nei titoli del documento. "Cantine Silvestro"
     * non viene creata come casa produttrice: i suoi 2 vini restano senza produttore.
     * "Tenuta Milina – Magnum" è la stessa casa produttrice di "Tenuta Milina" (il
     * Magnum è solo il nome della bottiglia).
     *
     * @return array<int, array{producer: string|null, wines: array<int, array{name: string, description: string}>}>
     */
    private function catalog(): array
    {
        return [
            [
                'producer' => 'Le Terre del Normanno',
                'wines' => [
...
FORMATO: 75 cl.
TEXT),
                    $this->wine('Aglianico Beneventano IGP', <<<'TEXT'
UVE: 100% Aglianico
VINIFICAZIONE: a temperatura controllata in acciaio inox
MATURAZIONE: in acciaio in grotti tufacee
...
FORMATO: 75 cl.
TEXT),
                    $this->wine('Falanghina Beneventano IGP', <<<'TEXT'
UVE: 100% Falanghina
VINIFICAZIONE: a temperatura controllata in acciaio inox
MATURAZIONE: in acciaio in grotti tufacee
GRADAZIONE ALCOLICA: 12%
