...
export function productCard(product) {
    const categoryIdentity = `${product.category?.name || ''} ${product.category?.slug || ''}`.toLocaleLowerCase('it-IT');
    const isWine = ['vino', 'vini', 'wine', 'cantina', 'enoteca'].some(alias => categoryIdentity.includes(alias));
    const wineClass = isWine ? ' wine-product' : '';
